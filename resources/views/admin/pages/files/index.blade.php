...
@section('content')
    <nav class="mb-6 ps-1" aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">
                <a href="{{ cms_route('pages.index', [$foreignModel->menu_id]) }}">Pages</a>
            </li>
    ...
    <div id="files-block">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center row-gap-4 gap-4 mb-6">
            ...
            <a href="{{ cms_route('pages.edit', [$foreignModel->menu_id, $foreignModel->id]) }}" class="ms-md-auto text-dark">
                ...
                <div id="item{{ $item->id }}" class="item col-6 col-md-4 col-lg-2" data-id="{{ $item->id }}" data-pos="{{$item->position}}">
                    <div class="card handle">
                        ...
                        <div class="py-3 px-5 d-flex gap-6">
                            <a href="#" data-url="{{cms_route('pages.files.edit', [$item->page_id, $item->id])}}"
                               class="item-edit" title="{{trans('general.edit')}}">
                                <i class="icon-base fa fa-pencil icon-xs"></i>
                            </a>
                            {{ html()->form('put', cms_route('pages.files.visibility', [$item->id]))
                            ...
                            {{ html()->form()->close() }}
                            {{ html()->form('delete', cms_route('pages.files.destroy', [$item->page_id, $item->id]))
...
@push('body.bottom')
    @include('admin.-scripts.files-index', [
        ...
        'routePositions' => cms_route('pages.files.positions', [$foreignModel->id]),
        'sort' => 'desc',
