                ...
                <div class="row">
                    ...
                    <div class="col-md-2 pe-0 mb-2">
                        {{ html()->select('suspended', [
                            ...
                            '0' => 'Active',
                            '1' => 'Suspended'
                        ], request('suspended'))->class('form-control') }}
                    </div>
                    <div class="col-md-auto">
                        <button type="submit" class="btn btn-primary">Search</button>
                        <a href="{{cms_route('cmsUsers.index', request()->only(['role']))}}" class="btn btn-label-secondary vat">Reset</a>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div class="table-responsive text-nowrap">
            <table class="table table-striped table-hover">
                ...
                @foreach($items as $item)
                    <tr>
                        <td>
                            <img src="{{ cms_route('cmsUsers.photo', [$item->id]) }}" width="20" height="20" alt="Photo" class="rounded-circle me-4" />
                        ...
                        <td><span class="badge bg-label-{{ $item->suspended ? 'warning' : 'primary' }} me-1">{{ $item->suspended ? 'Suspended' : 'Active' }}</span></td>
                        <td>{{ $item->id }}</td>
                        <td>
                            <div class="dropdown">
