            ...
            <div class="card">
                <div class="card-header header-elements flex-column flex-md-row align-items-md-center align-items-start gap-4">
                    ...
                    <div class="card-header-elements ms-md-auto flex-row-reverse flex-md-row">
                        ...
                        <a href="{{ cms_route('events.create', [$parent->id]) }}" class="btn btn-primary">
                    ...
                    <ul class="list-group list-group-flush" id="sortable">
                        ...
                        @foreach($items as $item)
                            <li id="item{{ $item->id }}" class="item list-group-item ps-0 d-flex justify-content-between align-items-center"
                                ...
                                <div>
                                    <i class="drag-handle cursor-move icon-base fa fa-bars icon-sm align-text-bottom me-1"></i>
                                ...
                                <div class="actions d-flex align-items-center gap-4">
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            {{ html()->form()->close() }}
                                            <a href="{{ cms_route('events.files.index', [$item->id]) }}" class="dropdown-item">
                                            ...
                                            {{ html()->form('delete', cms_route('events.destroy', [$item->collection_id, $item->id]))->class('form-delete')->open() }}
...
@endsection
@push('body.bottom')
    @include('admin.-scripts.transfer', ['route' => cms_route('events.transfer', [$parent->id]), 'column' => 'collection_id', 'list' => $parentTypes, 'parentId' => $parent->id])
    @if ($parent->admin_order_by == 'position')
        <script src="{{ asset('assets/vendor/libs/sortablejs/sortable.js') }}"></script>
        <script type="text/javascript">
            $(function () {
                sortable('{{cms_route('events.positions')}}', '{{csrf_token()}}', '{{ $parent->admin_sort }}', {{ $currentPage }});
            });
        </script>
    @endif
@endpush
