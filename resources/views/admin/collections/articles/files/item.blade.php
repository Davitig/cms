@if (! empty($item) || ! empty($itemInput))
    <li id="item{{$item->id}}" data-id="{{$item->id}}" data-pos="{{$item->position}}"
        data-url="{{cms_route('articles.files.edit', [$item->article_id, $item->id])}}" class="item col-md-2 col-sm-4 col-xs-6">
        <div class="album-image">
            ...
            <div class="image-options">
                ...
                <a href="#" data-url="{{cms_route('articles.files.visibility', [$item->id])}}" class="visibility">
                    <i class="fa fa-eye{{$item->visible ? '' : '-slash'}}"></i>
                </a>
                <a href="#" data-modal="edit"><i class="fa fa-pencil"></i></a>
                <a href="#" data-delete="{{cms_route('articles.files.destroy', [$item->article_id, $item->id])}}" data-id="{{$item->id}}"><i class="fa fa-trash"></i></a>
            </div>
        </div>
    </li>
@endif
