...
@section('content')
    <nav class="mb-6 ps-1" aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">
                <a href="{{ cms_route('articles.index', [$foreignModel->collection_id]) }}">Articles</a>
            </li>
    ...
    <div id="files-block">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center row-gap-4 gap-4 mb-6">
            ...
            <a href="{{ cms_route('articles.edit', [$foreignModel->collection_id, $foreignModel->id]) }}" class="ms-md-auto text-dark">
            ...
            <div class="d-flex btn-group" role="group">
                ...
                <button type="button" class="btn btn-outline-secondary text-dark" data-delete-selected
                        data-url="{{cms_route('articles.files.destroy_many', [$foreignModel->id])}}">
                ...
                <div id="item{{ $item->id }}" class="item col-6 col-md-4 col-lg-2" data-id="{{ $item->id }}" data-pos="{{$item->position}}">
                    <div class="card handle">
                        ...
                        <div class="py-3 px-5 d-flex gap-6">
                            <a href="#" data-url="{{cms_route('articles.files.edit', [$item->article_id, $item->id])}}"
                               class="item-edit" title="{{trans('general.edit')}}">
                                <i class="icon-base fa fa-pencil icon-xs"></i>
                            </a>
                            {{ html()->form('put', cms_route('articles.files.visibility', [$item->id]))
                            ...
                            {{ html()->form()->close() }}
                            {{ html()->form('delete', cms_route('articles.files.destroy', [$item->article_id, $item->id]))
...
@push('body.bottom')
    @include('admin.-scripts.files-index', [
        ...
        'routePositions' => cms_route('articles.files.positions', [$foreignModel->id]),
        'sort' => 'desc',
