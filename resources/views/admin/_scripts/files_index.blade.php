<script type="text/javascript">
    $(function () {
        ...
        // delete item(s)
        filesBlock.on('click', '[data-delete]', function (e) {
            e.preventDefault();

            let target = $(this);
            let input = {'_method':'delete', '_token':"{{$csrfToken = csrf_token()}}"};
            let deleteSelected = target.data('delete-selected');
            ...
            $.post(url, input, function (res) {
                if (deleteSelected && input.ids.length) {
                    $.each(input.ids, function(i, e) {
                        $('#item' + e, filesBlock).remove();
                    });
                } else {
        ...
        // visibility on/off
        filesBlock.on('click', '.visibility', function (e) {
            e.preventDefault();
            let item = $(this);

            let input = {'_token': '{{$csrfToken}}', '_method': 'put'};
            $.post(item.data('url'), input, function (res) {
                item.find('i').attr('class', res.data ? 'fa fa-eye' : 'fa fa-eye-slash');
