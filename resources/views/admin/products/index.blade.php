        ...
        <div class="table-responsive text-nowrap">
            <table class="table table-striped table-hover">
                ...
                @foreach($items as $item)
                    <tr>
                        <td>
                            <img src="{{ cms_route('cmsUsers.photo', [$item->id]) }}" width="40" height="40" alt="Photo" class="rounded-2 me-4" />
                            {{ $item->title }}
                        </td>
                        <td>
                            <i class="icon-base fa-regular fa-circle-{{ $item->in_stock ? 'check' : 'xmark' }} text-{{ $item->in_stock ? 'success' : 'danger' }}"></i>
                        </td>
                        <td>{{ $item->price }}</td>
                        <td>{{ $item->quantity }}</td>
                        <td>{{ $item->id }}</td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <a href="{{ cms_route('products.edit', [$item->id]) }}" class="dropdown-item">
                                    ...
                                    <button type="submit" class="dropdown-item" title="{{trans('general.visibility')}}">
                                        <i class="icon-base fa fa-eye{{$item->visible ? '' : '-slash'}} icon-sm me-1_5"></i>
                                        Visibility
                                    </button>
                                    {{ html()->form()->close() }}
