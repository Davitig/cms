<div class="app-brand navbar-brand py-5{{ $cmsSettings->get('horizontal_menu') ? ' d-none d-xl-flex' : '' }}">
    <a href="{{ cms_route('dashboard') }}" class="app-brand-link">
        ...
        <span class="app-brand-text menu-text fw-bold fs-4{{ $cmsSettings->get('horizontal_menu') ? ' text-heading' : '' }}">CMS</span>
    </a>
    <a href="javascript:void(0);" class="layout-menu-toggle menu-link text-large ms-auto{{ $cmsSettings->get('horizontal_menu') ? ' d-xl-none' : '' }}">
        @if ($cmsSettings->get('horizontal_menu'))
            <i class="icon-base fa fa-times icon-sm d-flex align-items-center justify-content-center"></i>
        @else
